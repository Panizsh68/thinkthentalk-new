...
const getCollabSchema = (t: any) => z.object({
  ...
  fieldOfExpertise: z.string().min(1, t('registration.validation.required')),
  experience: z.string().optional(),
  whyJoin: z.string().min(1, t('registration.validation.required')),
    ...
    return (
      <div className="container max-w-3xl py-16 px-4">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold">Join Think Then Talk Team</h1>
        ...
        <Card className="rounded-[2rem] border-none shadow-lg">
          ...
          <CardFooter>
            <Button className="w-full h-12 rounded-xl font-bold" onClick={() => setStep(2)}>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="name" render={({ field }) => (
                  <FormItem><FormLabel>{t('contact.form.nameLabel')}</FormLabel><FormControl><Input {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="email" render={({ field }) => (
                  <FormItem><FormLabel>{t('contact.form.emailLabel')}</FormLabel><FormControl><Input dir="ltr" {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="mobile" render={({ field }) => (
                  <FormItem><FormLabel>{t('auth.mobileLabel')}</FormLabel><FormControl><Input dir="ltr" {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="fieldOfExpertise" render={({ field }) => (
                  <FormItem><FormLabel>{t('collaborate.form.expertise')}</FormLabel><FormControl><Input placeholder={t('collaborate.form.expertisePlaceholder')} {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
                )} />
              </div>

              <FormField control={form.control} name="experience" render={({ field }) => (
                <FormItem><FormLabel>{t('collaborate.form.experience')}</FormLabel><FormControl><Textarea rows={3} {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
              )} />

              <FormField control={form.control} name="whyJoin" render={({ field }) => (
                <FormItem><FormLabel>{t('collaborate.form.whyJoin')}</FormLabel><FormControl><Textarea rows={4} {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
              )} />

              <FormField control={form.control} name="availability" render={({ field }) => (
                <FormItem><FormLabel>{t('collaborate.form.availability')}</FormLabel><FormControl><Input {...field} className="rounded-xl" /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <div className="flex gap-4 pt-4">
                ...
                <Button type="submit" className="flex-1 h-12 rounded-xl font-bold shadow-md" disabled={isPending}>
