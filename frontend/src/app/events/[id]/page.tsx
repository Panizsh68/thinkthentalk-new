...
import type { EventTicketConfig, EventResource } from "@/lib/types";
...
export default function EventDetailPage() {
  ...
  const isLoading =
    isLoadingEvent || (isAuthenticated && isLoadingRegistrations);

  if (isLoading) {
    return (
      <div className="container max-w-screen-2xl py-8 md:py-10">
        <Skeleton className="mx-auto mb-8 aspect-video w-full max-w-6xl rounded-[2rem]" />
  ...
  const eventSummary = getLocalizedTextValue(
    event.summary,
    language,
    event.summary.en,
  );
  const eventDescription = getLocalizedTextValue(
    event.description,
    language,
  ...
  return (
    <>
      ...
      <div className="container max-w-screen-2xl pb-8 pt-8 md:pb-12 md:pt-12">
        <div className="grid md:grid-cols-3 gap-8 lg:gap-12">
          {/* Main Content */}
          <div className="md:col-span-2 space-y-6">
        ...
        <div>
          ...
          <Badge variant="outline" className="mt-2 text-xs">
            {resource.accessLevel === "PUBLIC"
              ? t("event.resourcePublic")
              : t("event.resourceRegisteredOnly")}
          </Badge>
        </div>
      </div>
      ...
      {canAccess ? (
        ...
        <div className="flex flex-col items-center justify-center gap-2 text-center text-muted-foreground">
          ...
          <span className="text-xs max-w-[100px]">
            {t("event.resourceAvailableAfterRegistration")}
          </span>
        </div>
      )}
    </div>
