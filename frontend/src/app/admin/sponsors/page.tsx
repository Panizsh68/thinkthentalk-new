...
} from "@/hooks/use-sponsor-queries";
...
function AdminSponsorsPage() {
  ...
  const { mutateAsync: createSponsor, isPending: isCreating } =
    useCreateSponsorMutation({
      onSuccess: () => {
  ...
  const { mutateAsync: updateSponsor, isPending: isUpdating } =
    useUpdateSponsorMutation({
      onSuccess: () => {
  ...
  const { mutateAsync: deleteSponsor, isPending: isDeleting } =
    useDeleteSponsorMutation({
      onSuccess: () => {
  ...
  const handleFormSubmit = async (data: SponsorFormData) => {
    if (editingSponsor) {
      await updateSponsor({ id: editingSponsor.id, data });
    } else {
  ...
  const handleDeleteConfirm = async () => {
    if (sponsorToDelete) {
      const filePath = getUploadedFilePath(sponsorToDelete.logoUrl);
      try {
        ...
        if (filePath) {
          try {
            ...
            toast({
              variant: "destructive",
              title: t("admin.sponsors.delete.cleanupWarning"),
            });
          }
        }
      } catch {
    ...
    <div className="space-y-6">
                  ...
                  <TableRow key={sponsor.id}>
                    ...
                    <TableCell className="text-right">
                      <Button
                        variant="ghost"
                        size="icon"
                        aria-label={t("admin.sponsors.accessibility.edit")}
                      ...
                      <Button
                        ...
                        aria-label={t("admin.sponsors.accessibility.delete")}
      ...
      <SponsorFormDialog
        ...
        onCleanupWarning={() =>
          toast({
            variant: "destructive",
            title: t("admin.sponsors.form.cleanupWarning"),
          })
        }
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("admin.sponsors.delete.confirmTitle")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("admin.sponsors.delete.confirmText", {
