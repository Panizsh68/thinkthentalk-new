...
export class RegistrationsService {
  ...
  async getMyRegistrations(userId: string): Promise<UserRegistrationDto[]> {
    const entities =
      await this.registrationsRepository.findUserRegistrations(userId);
  ...
  async getAdminRegistrations(
    ...
    const entities =
      await this.registrationsRepository.findAllAdminRegistrations(filters);
  ...
  async exportAdminRegistrations(
    ...
    const entities =
      await this.registrationsRepository.findAllAdminRegistrations({
        ...filters,
  ...
  async getRegistrationDetails(
    ...
    const entity =
      await this.registrationsRepository.findByIdWithDetails(registrationId);
    if (!entity) {
  ...
  async updateRegistration(
    ...
    if (
      ...
      throw new BadRequestException(
        'Cannot change status from PAID to another state.',
      );
    }

    const updated = await this.registrationsRepository.updateRegistration(
      registrationId,
      {
