...
export class DiscountsService {
  ...
  status(): { status: 'ok'; module: string; timestamp: string } {
    return { status: 'ok', module: 'discounts', timestamp: new Date().toISOString() };
  }

  async listDiscounts(): Promise<DiscountDto[]> {
    const discounts = await this.prisma.discount.findMany({ include: { events: true } });
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      if (eventIds.length) {
        await tx.discountOnEvent.createMany({
          data: eventIds.map((eventId) => ({ discountId: discount.id, eventId })),
        });
      }
      return discount;
    });
  ...
  async updateDiscount(
    ...
    const eventIds = dto.applicableEventIds ?? existing.events.map((e) => e.eventId);
  ...
  async validateDiscount(userId: string, dto: ValidateDiscountDto): Promise<DiscountDto> {
    ...
    if (applicableEventIds.length > 0 && !applicableEventIds.includes(dto.eventId)) {
      throw new BadRequestException('Discount is not valid.');
    }

    if (discount.minAmount) {
      const minAmount =
        (discount.minAmount as any).toNumber?.() ?? (discount.minAmount as unknown as number);
  ...
  private toDiscountDto(
    discount: Prisma.DiscountGetPayload<{ include?: { events?: true } }> | Discount,
    ...
    return {
      ...
      value: (discount.value as any).toNumber?.() ?? (discount.value as unknown as number),
      applicableEventIds,
